...
def test_song_evaluation_bm_same():
  ...
  file_count_before = len([entry for entry in os.listdir(OUTPUT_FOLDER) if os.path.isfile(os.path.join(OUTPUT_FOLDER, entry))])
  
  expected_notation_string = ""
  actual_notation_strings = run_main_song_evaluation(exp_score, giv_score, True)

  file_count_after = len([entry for entry in os.listdir(OUTPUT_FOLDER) if os.path.isfile(os.path.join(OUTPUT_FOLDER, entry))])
