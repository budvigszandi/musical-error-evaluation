...
class RhythmEvaluationStats:
  ...
  def __init__(self, exp_count, giv_count):
    ...
    self.slightly_shorter_rhythm_count = 0       # Amount of rhythms that were substituted with a slightly shorter rhythm
    self.slightly_shorter_rhythm_percentage = 0  # Percentage of the rhythmic length lost by the slightly shorter rhythms (in respect to the whole expected length)
    self.slightly_longer_rhythm_count = 0        # Amount of rhythms that were substituted with a slightly longer rhythm
    self.slightly_longer_rhythm_percentage = 0   # Percentage of the rhythmic length gained by the slightly longer rhythms (in respect to the whole expected length)
  ...
  def __str__(self):
    ...
    string_rep += f"    This is {self.insertions_percentage:.2f}% of the whole.\n"
    string_rep += f"  Lengths (in quarters) of deleted chunks: {deletion_string}\n"
    string_rep += f"    This is {self.deletions_percentage:.2f}% of the whole.\n"
    string_rep += f"  Length (in quarters) of sounds substituted with rests: {self.note_rhythm_switch_length}\n"
    string_rep += f"    This is {self.note_rhythm_switch_percentage:.2f}% of the whole.\n"
    string_rep += f"  Length (in quarters) of rests substituted with sounds: {self.rhythm_note_switch_length}\n"
    string_rep += f"    This is {self.rhythm_note_switch_percentage:.2f}% of the whole.\n"
    string_rep += f"  Amount of slightly shorter rhythms: {self.slightly_shorter_rhythm_count}\n"
    string_rep += f"    These make it {self.slightly_shorter_rhythm_percentage:.2f}% shorter.\n"
    string_rep += f"  Amount of slightly longer rhythms: {self.slightly_longer_rhythm_count}\n"
    string_rep += f"    These make it {self.slightly_longer_rhythm_percentage:.2f}% longer.\n"
    string_rep += f"  Amount of vastly shorter rhythms: {self.vastly_shorter_rhythm_count}\n"
    string_rep += f"    These make it {self.vastly_shorter_rhythm_percentage:.2f}% shorter.\n"
    string_rep += f"  Amount of vastly longer rhythms: {self.vastly_longer_rhythm_count}\n"
    string_rep += f"    These make it {self.vastly_longer_rhythm_percentage:.2f}% longer.\n"
    string_rep += f"  The given length is {self.total_length_difference_percentage:.2f}% of the expected length.\n"
