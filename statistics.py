...
def get_final_song_stats(song_stats, exp_total_rhythmic_length, giv_total_rhythmic_length, matched_length, unmatched_length):
  ...
  merged_note_stats = get_merged_note_stats(song_stats.exp_count, song_stats.giv_count, song_stats.note_eval_stats)
  merged_rhythm_stats = get_merged_rhythm_stats(song_stats.exp_count, song_stats.giv_count, exp_total_rhythmic_length, giv_total_rhythmic_length, song_stats.rhythm_eval_stats)
...
def get_merged_note_stats(exp_count, giv_count, note_stats_list):
  ...
  for note_stat in note_stats_list:
    ...
    perfect_match_percentages.append(note_stat.perfect_match_percentage)
    merged_note_stats.cent_differences += note_stat.cent_differences
    cent_diff_percentages.append(note_stat.cent_diff_percentage)
    merged_note_stats.harmonics = list(map(add, merged_note_stats.harmonics, note_stat.harmonics))
    harmonic_exp_percentages.append(note_stat.harmonic_exp_percentage)
    harmonic_giv_percentages.append(note_stat.harmonic_giv_percentage)
    merged_note_stats.unrelated += note_stat.unrelated
    merged_note_stats.got_lowest_count += note_stat.got_lowest_count
    uncovered_percentages.append(note_stat.uncovered_percentage)
    covered_only_with_harmonics_percentages.append(note_stat.covered_only_with_harmonics_percentage)
    multiply_covered_percentages.append(note_stat.multiply_covered_percentage)
  ...
  merged_note_stats.perfect_match_percentage = "{:.2f}".format(merged_note_stats.unrelated / giv_count)
...
def get_merged_rhythm_stats(exp_count, giv_count, exp_total_rhythmic_length, giv_total_rhythmic_length, rhythm_stats_list):
  ...
  slighthly_shorter_rhythm_percentages = []
  slighthly_longer_rhythm_percentages = []
  ...
  for rhythm_stat in rhythm_stats_list:
    ...
    merged_rhythm_stats.slighthly_shorter_rhythm_count += rhythm_stat.slighthly_shorter_rhythm_count
    merged_rhythm_stats.slighthly_longer_rhythm_count += rhythm_stat.slighthly_longer_rhythm_count
  ...
  if exp_total_rhythmic_length != 0:
    ...
    merged_rhythm_stats.slighthly_shorter_rhythm_percentage = float("{:.2f}".format((sum(slighthly_shorter_rhythm_percentages) / len(slighthly_shorter_rhythm_percentages)) * 100))
    merged_rhythm_stats.slighthly_longer_rhythm_percentage = float("{:.2f}".format((sum(slighthly_longer_rhythm_percentages) / len(slighthly_longer_rhythm_percentages)) * 100))
    merged_rhythm_stats.vastly_shorter_rhythm_percentage = float("{:.2f}".format((sum(vastly_shorter_rhythm_percentages) / len(vastly_shorter_rhythm_percentages)) * 100))
    merged_rhythm_stats.vastly_longer_rhythm_percentage = float("{:.2f}".format((sum(vastly_longer_rhythm_percentages) / len(vastly_longer_rhythm_percentages)) * 100))
    ...
    merged_rhythm_stats.deletions_percentage = 0
    merged_rhythm_stats.note_rhythm_switch_percentage = 0
    merged_rhythm_stats.rhythm_note_switch_percentage = 0
    merged_rhythm_stats.slighthly_shorter_rhythm_percentage = 0
    merged_rhythm_stats.slighthly_longer_rhythm_percentage = 0
    merged_rhythm_stats.vastly_shorter_rhythm_percentage = 0
    merged_rhythm_stats.vastly_longer_rhythm_percentage = 0
...
def get_song_chunk_note_eval_stats(expected, given, scenario, note_evals):
  ...
  for i in range(len(scenario)):
    ...
    if current_step == DistanceType.DELETION:
      ...
      note_eval_stats = get_note_eval_stats(current_source, current_target, note_eval, 256)
      song_chunk_note_stats.perfect_matches += note_eval_stats.perfect_matches
      perfect_match_percentages.append(note_eval_stats.perfect_match_percentage)
      song_chunk_note_stats.cent_differences += note_eval_stats.cent_differences
      cent_diff_percentages.append(note_eval_stats.cent_diff_percentage)
      song_chunk_note_stats.harmonics = list(map(add, song_chunk_note_stats.harmonics, note_eval_stats.harmonics))
      harmonic_exp_percentages.append(note_eval_stats.harmonic_exp_percentage)
      harmonic_giv_percentages.append(note_eval_stats.harmonic_giv_percentage)
...
def get_stat_elem_count(simplified_data):
  if isinstance(simplified_data, list):
    count = 0
    for d in simplified_data:
      if d.isRest or d.isNote:
        ...
        count += len(d)
    return count
  elif simplified_data.isRest or simplified_data.isNote:
    return 1
  elif simplified_data.isChord:
    return len(d)

def get_note_eval_stats(expected_notes, given_notes, scenario, points):
  # TODO: Return something if scenario is None (only rhythmic difference)
  if isinstance(expected_notes, list):
    note_eval_stats = NoteEvaluationStats(len(expected_notes), len(given_notes))
    exp_notes_divisor = len(expected_notes)
    giv_notes_divisor = len(given_notes)
  elif expected_notes.isChord and given_notes.isChord:
    note_eval_stats = NoteEvaluationStats(len(expected_notes), len(given_notes))
    exp_notes_divisor = len(expected_notes)
    giv_notes_divisor = len(given_notes)
  elif expected_notes.isChord:
    note_eval_stats = NoteEvaluationStats(len(expected_notes), 1)
    exp_notes_divisor = len(expected_notes)
    giv_notes_divisor = 1
  elif given_notes.isChord:
    note_eval_stats = NoteEvaluationStats(1, len(given_notes))
    exp_notes_divisor = 1
    giv_notes_divisor = len(given_notes)
  else:
    note_eval_stats = NoteEvaluationStats(1, 1)
    exp_notes_divisor = 1
    giv_notes_divisor = 1
  ...
  for rel in scenario:
    ...
    if isinstance(expected_notes, list) or expected_notes.isChord:
      if rel.expected_note == expected_notes[0] and rel_type == NoteRelationshipType.PERFECT_MATCH:
        note_eval_stats.got_lowest = True
    elif expected_notes.isNote and rel_type == NoteRelationshipType.PERFECT_MATCH:
      note_eval_stats.got_lowest = True
  
  note_eval_stats.perfect_match_percentage = "{:.2f}".format((len(perfect_match_notes) / exp_notes_divisor) * 100)
  note_eval_stats.cent_diff_percentage = "{:.2f}".format((len(cent_diff_notes) / exp_notes_divisor) * 100)
  note_eval_stats.harmonic_exp_percentage = "{:.2f}".format((len(harmonic_exp_notes) / exp_notes_divisor) * 100)
  note_eval_stats.harmonic_giv_percentage = "{:.2f}".format((sum(note_eval_stats.harmonics) / giv_notes_divisor) * 100)
  note_eval_stats.unrelated_percentage = "{:.2f}".format((note_eval_stats.unrelated / giv_notes_divisor) * 100)
  note_eval_stats.uncovered_notes = get_uncovered_notes(expected_notes, scenario)
  note_eval_stats.uncovered_percentage = "{:.2f}".format((len(note_eval_stats.uncovered_notes) / exp_notes_divisor) * 100)
  note_eval_stats.covered_only_with_harmonics = get_covered_only_with_harmonics(expected_notes, scenario)
  note_eval_stats.covered_only_with_harmonics_percentage = "{:.2f}".format((len(note_eval_stats.covered_only_with_harmonics) / exp_notes_divisor) * 100)
  note_eval_stats.multiply_covered_notes = get_multiply_covered_notes(expected_notes, scenario)
  note_eval_stats.multiply_covered_percentage = "{:.2f}".format((len(note_eval_stats.multiply_covered_notes) / exp_notes_divisor) * 100)
